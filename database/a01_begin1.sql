...
# Database
1. 	데이터 베이스란 무엇인가?
	- 데이터베이스는 여러 사람이 [공유]하여 사용할 목적으로 [체계화]를 통해 [관리]하는 데이터의 집합이다.
	  작성된 목록으로써 여러 응용 시스템들의 [통합된 정보]들을 [저장]하여 [운영]할 수 있는 공용 데이터들의 묶음이다.(위키백과)
...
2. 	웹 프로그래밍에서 데이터베이스의 역할과 데이터베이스 식별
	1) 고정되어 유지하는 데이터를 체계적으로 저장 관리하는 역할을 한다.
		...
		웹화면에 jsp로 요청 category=200002939 전달하며, 
		database에 해당 category=200002939 정보를 전달
...
1. DBMS(database management system) : 
 		...
 		구성된 소프트웨어들의 집합체
 		 - 	데이터와 응용프로그램의 중간에서 응용프로그램이 요구하는 대로 데이터를 정의하고, 읽고, 쓰고
 			갱신하는 등의 데이터 조작하고 이들을 효율적으로 관리하는 프로그램들을 지칭한다.
...
5. sql 명령문의 종류
	...
	2) 데이터 조작어 (DML : data manipulation language) : 데이터를 입력/수정/삭제
		...
		ex) insert 테이블 values(데이터1, 데이터2...);
