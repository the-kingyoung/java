...
--ex) job이 SALESMAN이거나, MANAGER인 경우를 IN구문을 이용해서
SELECT *
FROM EMP
WHERE job IN ('SALESMAN','MANAGER');
--SUBQUERY로 활용해서 최고 연봉자인 사람의 정보 출력하세요.
--						부서번호가 30인 사람의 연봉 중 같은 사람을 출력하세요.
...
1. 	컬럼에 저장된 문자열 중에서 like 연산자에서 지정한 문자 패턴과 부분적으로 일치하면 
	...
	2) A시작하여 검색 : 검색컬럼like'A%'
	3) N로 끝나는 검색 : 검색컬럼 like '%N'
	4) 자리수 위치에 맞는 데이터 검색, 세번째 자리에 A나오는 데이터 검색:검색컬럼 like '___A%' 
 */
...
--deptno 기준으로 오름차순, job기준 내림차순으로 정렬하여 조회
...
FROM emp
ORDER BY sal;
--ex3) 부서번호 deptno기준 내림차순, sal기준 오름 차순으로 정렬처리하세요.
SELECT deptno, sal
FROM emp
