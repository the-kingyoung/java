...
SELECT empno, job, sal, deptno	--selsct 컬럼명1, 컬럼명2..
FROM emp;
...
select 컬럼명 as 별칭명, 컬럼명 별칭명
 	1)	기본 테이블에서 지정한 컬럼명이외에 다른 이름으로 컬럼명을 위 형식으로 처리하면,
 		출력할 수 있다.
 	2)	컬럼명안에 공백이나 특수문자(#,@)를 활용할 때는 "별칭" 형식으로 쌍따옴표	
 		사이에 별칭을 지정하면 된다.
 **/
...
--ex) 사원테이블에 사원번호(empno), 사원명(ename), 관리자번호(mgr),급여(sal)의 별칭으로 데이터를 표현하세요
 SELECT empno "사원 번호", ename "사 원 명", mgr "관리자 번호", sal "급 여"
 FROM emp;

--연결이 끊기시는 부분 : 상단에 선택해서 연결처리하시고, sql처리하면 됩니다.
/*
# 데이터의 컬럼간의 문자열 연결과 산술 연산 처리..
1.	문자열형 컬럼은 문자열컬럼1 || 문자열 컬럼2 형식으로 연결하면, 
'	해당 컬럼에 있는 데이터가 연결되어 출력된다.
	연결된 내용은 하나의 컬럼으로 선언하여 출력이 가능하다.
2.	숫자형 데이터도 문자열과 함께 ||를 활용하면, 문자열로 전환된 데이터를 확인할 수 있다.
3.	숫자형 데이터는 컬럼끼리, 또는 추가 데이터를 통해 연산이 가능하다.
 * */
SELECT ename, job, ename||' - '||job "# 연결문자 #"
FROM emp;
...
SELECT ename "사원명", sal "급여", sal*0.1 "급여의 10%", ROUND((sal/13),2) "연봉의 1/13"
FROM emp;

SELECT ename|| '의 사원번호는 '||empno||'이고, 10%인상된 급여는 '||(sal+(sal*0.1))|| '만원 이다.'"급여 정보"
FROM emp;
-- ex1) ename(empno)의 월급여는 sal/13 입니다. 를 한문장으로 출력하세요.
--		round() : 소수점이하 절삭(함수시 진행)
SELECT ename||'('||empno||') 의 월급여는 '||round((sal/13),2)||'입니다.'
FROM emp;
-- ex2) ename님의 이달 보너스는 sal의 deptno%기준으로 sal*(deptno/100)만원이다.
SELECT ename||'님의 이달 보너스는 '||sal||'의 '||deptno||'% 기준으로'||(sal*(deptno/100))||'만원이다.'show
FROM emp;
/*
 # SQL명령문의 표준 형식..
 1.	sql명령문
...
-- 각 deptno를 데이터별로 그룹지어 최고값의 sal를 출력
...
-- ex3) emp테이블 job별 최고 연봉 (sal)를 출력하세요
...
WHERE deptno = 10;
/*
2. where 절의 조건문은 컬럼 이름, 연산자, 상수, 산술 표현식을 결합하여 다양한 형태로 표현한다.
...
WHERE deptno=10
OR sal>=3000;
--ex) job이 'SALESMAN'인 데이터를 출력하되, 컬럼은 ENAME, JOB,SAL을 선택하세요
SELECT ENAME, JOB,SAL
FROM EMP
WHERE job='SALESMAN';
--ex) 부서번호(deptno)가 20이고, sal이 4000미만인 데이터를 출력하세요
SELECT *
FROM emp 
WHERE deptno=20
AND sal<4000;
...
--ex1) job이 'CLERK'가 아닌 데이터를 출력하세요
SELECT * FROM EMP 
WHERE job != 'CLERK';
