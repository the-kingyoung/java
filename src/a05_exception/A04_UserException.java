...
3.	기본형식.
	...
	2) 사용자 정의 예외 클래스 호출.
		...
		try{
			if(조건){
				throw new 사용자 정의 예외 생성자(매개데이터)
			}
		}catch(사용자 정의 예외 클래스 e ){
			System.out.println("사용자 정의 예외 발생");
			System.out.println(e.getMessage());
			System.out.println(e.선언한 기능메서드());
		}
	...
	try {
		for (int cnt = 1; cnt < 10; cnt++) {
			if (cnt == 5) {
				...
				throw new UserException("숫자가 5일때 예외!");
...
//ex) UserException2로 사용자정의 예외를 만들고,
//	  생성자 매개변수로 문자열을 전달,
//	  getMessage()오버로딩으로 "사용자 정의 2" 추가 처리..
...
//	  main()에서 반복을 통해서 짝수일 때, 해당 내용 호출 처리..	
	...
	for (int i = 1; i < 10; i++) {
		...
		try {
			...
			if (i % 2 == 0) {
				throw new UserException2("짝수 발생!!");
			}
		} catch (UserException2 e1) {
...
class UserException2 extends Exception{
	...
	public void call2() {
		System.out.println("두번째 예외 클래스입니다.!!^^");
	}
