 ...
 *1. 객체를 생성할 때, heap영역의 주소값으로 stack 영역에 할당하는데,
 *	 이 주소값을 할당하는 변수를 reference variables(참조변수)라고한다. 
 *
 *2. default데이터
 *		1) 데이터를 할당하지 않을 default로 할당되는 데이터를 말한다.
 *		2) primitive데이터 타입을
 ...
 *
 *3.객체지향 프로그램이 아닌 경우, 메모리에 관련된 코드를 처리하여야 하지만
 *	java는 jvm에서 가배지 컬렉터에서 사용되지 않는 메모리는 자동으로 해제해주는 처리를 한다.
 */
		//default값 확인
		Test t1 = new Test();
		...
		//System.out.println(t1.name.toString());//객체는 null
		//객체가 생성되지 않는 상황에서 객체가 생성되었을 때, 사용될 수 있는
		//필드나 메서드를 호출하면 NullPointerException이 발생한다.
	}
