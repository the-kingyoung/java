 ...
 * 2. 특정한 에러나 문제가 발생하면 해당 라인에서 프로세서가 중지가 되고,
 * 		예외를 던져 그 이후에 코드는 처리되지 않는다.
 ...
 * 		}else if(boolean4){
 * 			boolean3이 false이고, boolean4가 true일 때, 처리할 코드
 * 		}else{
 * 			위에 나열된 boolean3과 boolean4에 해당하지 않을 때, 처리할 코드
 * 		}
 * 
 * 		ex) 조건1 && 조건2,
 * 			조건1 || 조건2,
 ...
 * 			
 * 
 * ex) num01에 1-4중에 임의의 데이터를 할당하고.
 * 	   num02에 1-4중에 임의의 데이터를 할당했을 때,
 * 	   동일하면 정답 그 외에는 오답으로 표기하세요 if else 활용
 * 
 * ex2) point 점수를 0-100까지 임의로 할당하고
 *		90이상일 때 a, 80이상일때b등 학점 계산하는 프로그램을
 *		만들어 출력하세요.
 */
