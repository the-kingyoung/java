 ...
 * 		플렛폼 독립적 : OS(operation system - 운영체제)에 상관없이 사용할 수 있다.
 * 		linus ==> Unix
 ...
 * 			- 클래스 객체, 일반 기본 유형
 * 		4) 데이타 타입 변환
 * 			-promote(작은컵에서 큰컵으로 물붓기), casting(큰컵에서 작은컵으로 물붓기)
 * 4. 연산자
 ...
 * 			==> 객체 범위로 들어간다.
 * 			==> heap영역을 활용하여 stack영역에는 참조변수를 활용한다.
 * 		2)기본 형식
 * 			int [] array = {100,200,300};
 * 			데이터 유형 [] 참조변수명 = {할당데이터1, 할당데이터2....};	
 * 
 * 7. 객체지향프로그래밍
 * 		1) class란?
 ...
 * 			메모리는 두개를 메모를 다 활용한다.
 * 			stack영역에는 heap의 주소값, heap영역에는 실제 객체를 할당.
 * 		3) 객체의  종류
 * 			-primitive데이터가 모인 배열
 * 			-java의 내장된 객체 : String(문자열) : 문자(char)가 모여서 된 배열(char[]),
 ...
 *				public 클래스명(){
 *				}
 *				//3. 메서드 : 객체의 메모리에 있는 내용을 할당하거나 동적인 기능 처리를
 *							할 때, 사용된다.
 ...
 * 			해당 구성된 클래스를 메모리에서 호출할 때, 1:1 관계의 내용을 사용하게 된다.
 * 			class Customer{
 * 				private Product pro;
 * 			}
 * 			class Product{}
 ...
 *	======================================================================중간단계
 *10. 예외 처리
 *		1) 특정 프로그래밍의 예외적(에러, 정상적이지 않는 작동)에 대비해서 브로그래밍의 신뢰성과
 *			안정성을 위해 만든 클래스를 말한다.
 ...
 *		4) 계층별 예외처리
 *		5) throws 예외의 위임
 *		6) throw new 사용자 정의 예외(); : 예외 강제 발생
 *		
 ...
 *
 *12. Collections
 *		1) ArrayList <데이터 유형> : add(), remove(), get()...
 *		2) Set : 메모리가 비유적으로 한다면 항아리 모양으로 되어 있어, 할당 순서를 확인할 수 없지만,
 *				 중복이 불가능하다.
 *		3) List : 메모리가 선형으로 순서를 지켜서 들어가면, 중복이 가능하다.
 *		4) Map : 메모리가 key, value형식으로 할당되어 있어, key값으로 value의
 *				 객체를 호출하여 사용할 수 있다.
 *13. Thread
 ...
 *		2) 기본적으로 Stream을 기반으로 InputStream, OutStream을 사용하며,
